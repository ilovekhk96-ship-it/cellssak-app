...
export default function EntrySheet({ mode, currentStatus, form, setForm, onClose, onSave, onDelete, confirmingDelete }) {
  const meta = STATUS[currentStatus] || STATUS.seed;
  const selectedStatus = mode === 'edit' ? form.status : currentStatus;
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center z-20" style={{ maxWidth: '384px', margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--paper)', fontFamily: 'var(--font-body)' }} className="relative w-full rounded-t-3xl px-5 pt-5 pb-8 flex flex-col gap-3.5">
        ...
        {mode === 'add' && (
          <p style={{ color: STATUS.seed.color, background: STATUS.seed.soft }} className="text-xs rounded-full px-3 py-1.5 w-fit">
            🌱 기도씨앗으로 등록돼요
          </p>
        )}
        {mode === 'edit' && (
